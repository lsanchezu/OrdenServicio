                    ...
                    <div class="row">
                       ...
                       <div class="col-sm-4">
                           <div class="form-group">
                               ...
                               <select name="cboContrato" v-model="solicitud.contratoDto.idContrato"
                                        v-validate="'select_required'"
                                        :class="{'form-control': true, 'error': errors.has('cboContrato') }">
                                   <option v-for="item in listaContratos" v-bind:value="item.idContrato" :key="item.idContrato">{{ item.descripcion }}</option>
                               </select>
                               <label class="error" v-show="errors.has('cboContrato')">{{ errors.first('cboContrato') }}</label>
                           </div>
                       </div>
                       <div class="col-sm-4">
                           <div class="form-group">
                       ...
                       <div class="col-sm-4">
                           <div class="form-group">
                               ...
                               <input type="text" id="order_id" name="order_id" v-model="solicitante" placeholder="Nombre del solicitante" class="form-control" maxlength="100" 
                               v-on:keyup.enter="agregarProveedorContratista()">
                           </div>
                       </div>
                       <div class="col-sm-4">
                           <div class="form-group">
                               ...
                               <input type="number" id="order_id" name="order_id" value="1" readonly placeholder="Número de solicitud" class="form-control" maxlength="100" 
                               v-on:keyup.enter="agregarProveedorContratista()">
                           </div>
                       </div>
                       <div class="col-sm-4">
                           <div class="form-group">
                               <label class="control-label" for="order_id">Número de change order y revisión </label>
                               <input type="text" id="order_id" name="order_id" readonly v-model="change" placeholder="Número de change order y revisión" class="form-control" maxlength="100" 
                               v-on:keyup.enter="agregarProveedorContratista()">
                           </div>
                       </div>
                       <!-- <div class="col-sm-4">
                           <div class="form-group">
                               <label class="control-label" for="customer">Tipo de cambio</label>
                               <select name="cboTipoCambio" v-model="solicitud.tipoCambioDto.idTipoCambio"
                                        v-validate="'select_required'"
                                        :class="{'form-control': true, 'error': errors.has('cboTipoCambio') }">
                                   <option v-for="item in listaTipoCambio" v-bind:value="item.idTipoCambio" :key="item.idTipoCambio">{{ item.descripcion }}</option>
                               </select>
                               <label class="error" v-show="errors.has('cboTipoCambio')">{{ errors.first('cboTipoCambio') }}</label>
                           </div>
                       </div> -->
                       <div class="col-sm-4">
                           <div class="form-group">
                               <label class="control-label" for="customer">Estimación de cambio</label>
                               <select name="cboEstimacion" v-model="solicitud.estimacionDto.idEstimacion"
                                        v-validate="'select_required'"
                                        :class="{'form-control': true, 'error': errors.has('cboEstimacion') }">
                                   <option v-for="item in listaEstimacion" v-bind:value="item.idEstimacion" :key="item.idEstimacion">{{ item.descripcion }}</option>
                               </select>
                               <label class="error" v-show="errors.has('cboCompania')">{{ errors.first('cboCompania') }}</label>
                           </div>
                       </div>
                       <div class="col-sm-4">
                           <div class="form-group">
                               <label class="control-label" for="customer">Modalidad de cambio</label>
                               <select name="cboModalidad" v-model="solicitud.modalidadDto.idModalidad"
                                        v-validate="'select_required'"
                                        :class="{'form-control': true, 'error': errors.has('cboModalidad') }">
                                   <option v-for="item in listaModalidad" v-bind:value="item.idModalidad" :key="item.idModalidad">{{ item.descripcion }}</option>
                               </select>
                               <label class="error" v-show="errors.has('cboModalidad')">{{ errors.first('cboModalidad') }}</label>
                           </div>
                       </div>
                       <div class="col-sm-12">
                           <div class="form-group">
                               ...
                               <textarea name="txtDenominacionServicio"  
                                       ...
                                       placeholder="Ingrese el detalle (cant. max. 1024)" 
                       ...
                       <div class="col-sm-6">
                        <label class="control-label" for="txtDenominacionServicio">Efecto de cambio en el alcance</label> 
                        <div>
                            <label>
                            <input type="checkbox" v-model="cambioAlcance" />
                            Cambio en el alcance
                            </label>
                        </div>
                        <div>
                            <label>
                            <input type="checkbox" v-model="cambioPresupuesto" />
                            Cambio interno de presupuesto (budget shift)
                            </label>
                        </div>
                        <div>
                            <label>
                            <input type="checkbox" v-model="alcanceAdicional" />
                            Alcance adicional
                            </label>
                        </div>
                        <div>
                            <label>
                            <input type="checkbox" v-model="otro" @change="toggleOtro" />
                            Otro
                            </label>
                            <div v-if="showOtro">
                            <textarea v-model="otroDescripcion" placeholder="Especifica otro cambio..."></textarea>
                            </div>
                        </div>
                       </div>
                       <div class="col-sm-6">
                        <label class="control-label" for="txtDenominacionServicio">Efecto de cambio en el cronograma</label> 
                        <div>
                            <label>
                            ¿Impacta a la fecha del fin del contrato?
                            <select v-model="impactaFinContrato">
                        ...
                        <div>
                            ...
                            <input type="date" class="form-control" id="fechaInicio" v-model="fechaInicio" ref="fechaInicio" placeholder="Selecciona la fecha" />
                        </div>
                        <div>
                            <label>
                            Fecha de culminación del cambio:
                            </label>
                            <input type="date" class="form-control" id="fechaInicio" v-model="fechaInicio" ref="fechaInicio" placeholder="Selecciona la fecha" />
                        </div>
                       </div>
                        <!-- <div class="col-sm-4">
                            <div class="form-group">
                                <label>
                                <input type="checkbox" v-model="impactoEfecto" />
                                IMPACTO - EFECTO EN EL ALCANCE
                                </label>
                                <p>Seleccionado: {{ impactoEfecto ? 'Sí' : 'No' }}</p>
                                <input
                                v-if="impactoEfecto"
                                type="text"
                                placeholder="Ingrese detalles sobre el impacto en el alcance"
                        ...
                        <div class="form-group">
                            <label>
                            <input type="checkbox" v-model="impactoEfectoMonto" />
                            IMPACTO - EFECTO EN EL MONTO DE CONTRATO
                            </label>
                            <p>Seleccionado: {{ impactoEfectoMonto ? 'Sí' : 'No' }}</p>
                            <input
                            v-if="impactoEfectoMonto"
                            type="text"
                            placeholder="Ingrese detalles sobre el impacto en el monto"
                        ...
                        <div class="form-group">
                            <label>
                            <input type="checkbox" v-model="impactoEfectoCronograma" />
                            IMPACTO - EFECTO EN EL CRONOGRAMA CONTRACTUAL
                            </label>
                            <p>Seleccionado: {{ impactoEfectoCronograma ? 'Sí' : 'No' }}</p>
                            <input
                            v-if="impactoEfectoCronograma"
                            type="text"
                            placeholder="Ingrese detalles sobre el impacto en el cronograma"
                       ...
                       <!-- <div class="col-sm-12">
                           <div class="form-group">
                               <label class="control-label" for="order_id">Otros Impactos </label>
                               <input type="text" id="order_id" name="order_id" v-model="otrosImpactos" placeholder="Otros Impactos" class="form-control" maxlength="100" 
                               v-on:keyup.enter="agregarProveedorContratista()">
                           </div>
                       </div> -->
                       <div class="col-sm-11">
                           <!-- <div class="form-group">
                               <label class="control-label" for="order_id">Resumen Valor Contractual </label>
    ...
    export default {
        ...
        data() {
            return {
                ...
                resumen: 'MONTO INICIAL DEL CONTRATO: 8578.00, MONTO DE LAS ORDENES DE CAMBIO ACUMULADAS: 5454.00, VALOR DE LA ORDEN DE CAMBIO ACTUAL: 58421.00, NUEVO VALOR CONTRACTUAL: 5456.00, INDICADOR PORCENTUAL: 98.00',
                listaFuenteContrato: [],
                listaProyecto: [],
                listaCompania: [],
                listaFaseProyecto: [],
                ...
                impactoEfecto: false,
                impactoEfectoMonto: false,
                impactoEfectoCronograma: false,
                listaContratos: [{idContrato: 0, descripcion: '::SELECCIONE::'},{idContrato: 2, descripcion: 'CONTRATO 1'}],
                listaRequeridos: [{idRequerido: 0, descripcion: '::SELECCIONE::'},{idRequerido: 2, descripcion: 'CLIENTE'}, {idRequerido: 3, descripcion: 'PROVEEDOR'}],
                listaTipoCambio: [{idTipoCambio: 0, descripcion: '::SELECCIONE::'},
                                    {idTipoCambio: 2, descripcion: 'CAMBIO DE ALCANCE'}, 
                                    {idTipoCambio: 3, descripcion: 'ALCANCE ADICIONAL'},
                                    {idTipoCambio: 4, descripcion: 'CAMBIO DE CRONOGRAMA'},
                                    {idTipoCambio: 5, descripcion: 'CAMBIO INTERNO EN EL PRESUPUERSTO (BUDGETSHIFT)'},
                                    {idTipoCambio: 6, descripcion: 'PRESUPUESTO ADICIONAL'},
                                    {idTipoCambio: 7, descripcion: 'OTROS'}],
                listaEstimacion: [{idEstimacion: 0, descripcion: '::SELECCIONE::'},{idEstimacion: 1, descripcion: 'MONTO FIJO'},{idEstimacion: 2, descripcion: 'MONTO ESTIMADO'}],
                listaModalidad: [{idModalidad: 0, descripcion: '::SELECCIONE::'},
                                {idEstimacion: 1, descripcion: 'SUMA ALZADA'},
                                {idEstimacion: 2, descripcion: 'TIEMPO Y MATERIALES'},
                                {idEstimacion: 3, descripcion: 'PRECIOS UNITARIOS / TARIFAS UNITARIAS'},
                                {idEstimacion: 4, descripcion: 'REEMOLSABLES'}
                ],
                initialSolicitud: {
                    tipoDto: { idTipo: 0 },
                    ...
                    tipoCambioDto: {idTipoCambio: 0},
                    estimacionDto: {idEstimacion: 0},
                    modalidadDto: {idModalidad: 0},
                ...
                cambioAlcance: false,
                cambioPresupuesto: false,
                alcanceAdicional: false,
                otro: false,
                otroDescripcion: '',
                showOtro: false,
                submitted: false,
                impactaFinContrato: 'no', // Valor por defecto
                fechaInicio: null,
        ...
        computed: {
            ...mapState({
                nombreApellido: state => state.nombreApellido,
                rol: state => state.rol
            }),
            activarRucEmpresa() {
        ...
        methods: {
            ...
            inicializarReporte(data) {
                ...
                data.solicitudOrdenServicio.tipoCambioDto = {idTipoCambio: 0}
                data.solicitudOrdenServicio.estimacionDto = {idEstimacion: 0}
                data.solicitudOrdenServicio.modalidadDto = {idModalidad: 0}
      ...
      if (!this.showOtro) {
        this.otroDescripcion = ''; // Limpiar el campo si se desmarca
      }
    },
        }
    }
...
<style scoped>
.input-right {
  margin-left: 10px; /* Espacio entre el checkbox y el input */
  display: inline-block; /* Asegura que el input esté en línea con el checkbox */
  width: auto; /* Ajusta el ancho del input según el contenido */
}
</style>
