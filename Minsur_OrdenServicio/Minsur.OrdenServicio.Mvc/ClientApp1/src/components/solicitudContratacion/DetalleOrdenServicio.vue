...
export default {
    ...
    props: {
        idSolicitudContratacion: {type: Number,required: false, default : 0}
    },
    data(){
        return {
            solicitud: {},
    ...
    methods: {
        ...
        inicializar(){
            var self = this
            SolicitudService.obtenerSolicitud({ 'idSolicitudContratacion': this.idSolicitudContratacion})
            .then(response => {
                self.solicitud = response.data.solicitud
            });
        },
        descargarArchivo(event, archivo){
            window.open(`/SolicitudContratacion/DescargarArchivo?idSolicitudArchivoAdjunto=${archivo.idSolicitudArchivoAdjunto}`);
        },
        decargarTodo(){
            window.open(`/SolicitudContratacion/DescargarTodo?numeroSolicitud=${this.solicitud.numeroSolicitud}`);
        },
        registrarValidacion(solicitudValidacion){
            var self = this

            solicitudValidacion.idSolicitudContratacionServicio = this.solicitud.idSolicitudContratacionServicio;
        ...
        registrarRecomendacion(solicitudRecomendacion){
            var self = this
            solicitudRecomendacion.idSolicitudContratacionServicio = this.solicitud.idSolicitudContratacionServicio;
        ...
        registrarAprobacionSolicitud(solicitudAutorizacion){
            var self = this
            solicitudAutorizacion.idSolicitudContratacionServicio = this.solicitud.idSolicitudContratacionServicio;
