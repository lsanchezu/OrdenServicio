<template>
     <form>
                        ...
                        <div class="col-sm-4" v-if="solicitud.monedaDto.idMoneda == OtraMoneda">
                            <div class="form-group">
                                ...
                                <label class="error" v-show="errors.has('txtOtraMoneda')">{{ errors.first('txtOtraMoneda') }}</label>
                                 <!-- v-validate="esRequeridoOtraMoneda" -->
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <div class="row">
                        <documento-adjunto v-for="(documento, index) in listaDocumentoAdjunto" :key="index" 
                                            :documento= documento 
...
<script>
    ...
    import DocumentoAdjunto from '@/components/SolicitudContratacion/DocumentoAdjunto'
    ...
    export default {
        ...
        methods: {
            ...
            inicializarReporte(data) {
                this.solicitud = data.solicitudContratacion
            ...
            obtenerSolicitudRequest() {
                let solicitudRequest = {
                    SolicitudContratacion: JSON.parse(JSON.stringify(this.solicitud))
                };

                const montoEstimadoFormateado = solicitudRequest.SolicitudContratacion.montoEstimado.toString().replace('.', ',');
                solicitudRequest.SolicitudContratacion.montoEstimado = montoEstimadoFormateado;
                ...
                SolicitudService.registrarSolicitud(formData).then(response => {
                    ...
                    else
                    {
                        SwalAlert.successAction(response.data.mensaje, function(){
                            self.$router.push({ name: 'consulta-solicitud' })
                        });
                    }
                })
            }
