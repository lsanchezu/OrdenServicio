<template>
    <div>
        <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-10">
                <h2>Consultar Orden de Servicio</h2>
                <ol class="breadcrumb">
                    <li>
                        <a href="">Home</a>
                    </li>
                            ...
                            <div class="ibox-content">
                                <div class="table-responsive m-t">
                                    <table class="table table-striped custom-table" >
                                        ...
                                        <tbody>
                                            <tr v-for="(item, index) in listaOrdenServicio" :key="index">
                                                ...
                                                <td class="text-center">
                                                    <button class="btn btn-success" @click="mostrarDetalleSolicitud(item.idSolicitudContratacionServicio)"><i class="fa fa-arrow-circle-right"></i></button>
                                                </td> 
                                                <td class="text-center">
                                                    <button class="btn btn-info" v-if="item.estadoDto.nombre === 'Aprobado' || item.estadoDto.nombre === 'Rechazado' " @click="descargarPdf(item.idSolicitudContratacionServicio, item.numeroSolicitud)" ><i class="fa fa-file-pdf-o"></i></button>
                                                </td> 
                                            </tr>
                                        </tbody>
                                    </table>
         ...
         <el-dialog
            ...
            <keep-alive :max="1">
                <component :is="currentView" v-if="dialogVisible" 
                            v-bind:idSolicitudContratacion ="idSolicitudContratacion" 
...
<script>
import DetalleOrdenServicio from '@/components/SolicitudContratacion/DetalleOrdenServicio'
...
export default {
    ...
    data(){
        return {
            selectRowIndex: null,
            dialogVisible: false,
            currentView: "DetalleSolicitudContratacion",
            idSolicitudContratacion: 0,
            listaCompania:[],
            listaFuenteContrato:[],
            listaProyecto:[],
            listaFaseProyecto:[],
    ...
    methods: {
        ...
        consultarSolicitud(nroPagina = 1) {
            ...
            SolicitudService.consultarSolicitud(filtroSolicitud)
                .then(response => {
                    self.listaOrdenServicio = response.data.listaOrdenServicio;
        ...
        exportarConsulta(){
            ...
            window.open(`/SolicitudContratacion/ConsultarExportPorSolicitud?idCompania=${this.idCompania}&idProyecto=${this.idProyecto}&idFaseProyecto=${this.idFaseProyecto}
        ...
        exportarConsultaPorProveedor(){
            ...
            window.open(`/SolicitudContratacion/ConsultarExportPorProveedor?idCompania=${this.idCompania}&idProyecto=${this.idProyecto}&idFaseProyecto=${this.idFaseProyecto}
                                                                &idFuenteContrato=${this.idFuenteContrato}&idAreaFuncional=${this.idAreaFuncional}&idEstado=${this.idEstado}
                                                                &fechaInicio=${this.fechaInicio}&fechaFin=${this.fechaFin}&token=${JwtService.obtenerToken()}`);
        },
        validarConsulta(){
        ...
        mostrarDetalleSolicitud(item){
            ...
            this.idSolicitudContratacion = item;
        ...
        descargarPdf(idSolicitudContratacionServicio, numeroSolicitud){
            axios({
                method: 'get',
                url: `/OrdenServicio/DescargarPdf?idSolicitudContratacion=${idSolicitudContratacionServicio}`,
                responseType: 'blob'
            })
            .then(response => {
